/**
 * Scans text for keywords and returns a suggested priority level.
 * Uses a weighted scoring system for better accuracy.
 * @param {string} text - The description to analyze
 * @returns {string} - 'Low', 'Medium', or 'High'
 */
export const detectPriority = (text) => {
  if (!text || text.length < 5) return 'Low';
  ...
  // Check for high weight terms (Immediate High)
